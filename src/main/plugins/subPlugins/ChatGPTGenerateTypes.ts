...
const buildPersistedTypesSystemPrompt = ({
  ...
  [
    ...
    '- `_inheritance` may also use the form `"type@key1,key2,key3"`.',
    '- In that form, the part before `@` is the parent `"type"` to match, and the comma-separated keys after `@` are the only top-level keys to keep from the matched parent `core`.',
    '- When `_inheritance` uses `@key1,key2`, inherit only those listed keys from the matched parent `core` and ignore the other parent `core` keys.',
    ...
    '- Example: if `potion` has `_inheritance: "item@uniqueKey,name,order"`, then inherit only `uniqueKey`, `name`, and `order` from `item.core`, then add `potion.core` fields such as `potionValue`.',
